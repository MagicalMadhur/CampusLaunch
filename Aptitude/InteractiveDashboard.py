...
# --- Streamlit UI Setup ---
st.set_page_config(page_title="Quiz Performance Dashboard", page_icon="📊", layout="wide")
...
with st.sidebar:
    st.header("Filters")
    # Read username from URL query params (passed from student dashboard)
    query_username = st.query_params.get("username", None)
    if query_username:
    ...
    # Auto-load data if username was passed from dashboard
    if query_username and not submit_button:
        submit_button = True
