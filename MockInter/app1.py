...
def record_audio():
    ...
    try:
        ...
        st.warning("Voice recording is unavailable because PyAudio is not installed. Type your answer instead.")
        return None
    except OSError as e:
        st.warning(f"Could not access the microphone: {e}. Type your answer instead.")
        return None
    except sr.WaitTimeoutError:
        st.warning("No speech detected before timeout. Try again or type your answer.")
        return None
    ...
    if index < len(interview["questions"]):
        ...
        with col1:
            if st.button("Record Answer"):
                recorded_answer = record_audio()
                if recorded_answer:
                    st.session_state.answer_text = recorded_answer
                st.rerun()
