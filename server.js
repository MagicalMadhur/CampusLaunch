...
const studentSchema = new mongoose.Schema({
    ...
    resumePath: String, // Path to uploaded resume PDF
    resumeOriginalName: String, // Original filename of the resume
});
...
// Student Registration Route (with resume upload)
app.post('/register', upload.single('resume'), async (req, res) => {
    try {
        ...
        // If resume file was uploaded, save its path
        if (req.file) {
            studentData.resumePath = req.file.filename;
...
// Resume Upload/Update for existing users
app.post('/students/:username/resume', upload.single('resume'), async (req, res) => {
    try {
        ...
        if (student.resumePath) {
            const oldFilePath = path.join(uploadsDir, student.resumePath);
            if (fs.existsSync(oldFilePath)) {
                fs.unlinkSync(oldFilePath);
            }
        }

        student.resumePath = req.file.filename;
...
app.get('/students/:username/resume', async (req, res) => {
    try {
        ...
        res.status(200).json({
            ...
            resumeOriginalName: student.resumeOriginalName || null
        });
    } catch (error) {
        res.status(500).json({ message: 'Error fetching resume info', error });
    }
});

// Download student resume
app.get('/students/:username/resume/download', async (req, res) => {
    try {
        const { username } = req.params;
        const student = await Student.findOne({ username });
        ...
        const filePath = path.join(uploadsDir, student.resumePath);
        if (!fs.existsSync(filePath)) {
            return res.status(404).json({ message: 'Resume file not found on server' });
        }
        res.download(filePath, student.resumeOriginalName || 'resume.pdf');
    } catch (error) {
...
app.delete('/students/:username/resume', async (req, res) => {
    try {
        const { username } = req.params;
        const student = await Student.findOne({ username });
        if (!student) {
        ...
        student.resumeOriginalName = null;
        await student.save();
...
// JSearch API Proxy for Job Search
app.get('/api/jobs/search', async (req, res) => {
    try {
        const { query, page, num_pages, employment_types, date_posted } = req.query;
...
app.get('/api/resume/skills', async (req, res) => {
    try {
        ...
        const student = await Student.findOne({ username });
        if (!student || !student.resumePath) {
            return res.status(404).json({ message: 'No resume found', skills: [] });
        }

        const resumeFullPath = path.join(__dirname, student.resumePath);
        if (!fs.existsSync(resumeFullPath)) {
            return res.status(404).json({ message: 'Resume file not found', skills: [] });
        }

        // Read PDF text
        const pdfParse = require('pdf-parse');
        const dataBuffer = fs.readFileSync(resumeFullPath);
        ...
        const skillsList = [
            ...
            'tableau', 'power bi', 'excel', 'hadoop', 'spark',
            'blockchain', 'cybersecurity', 'cloud computing',
            'agile', 'scrum', 'jira',
        ];

        const found = skillsList.filter(skill => text.includes(skill));
        res.status(200).json({ skills: found, count: found.length });
    } catch (error) {
        console.error('Skills extraction error:', error.message);
        res.status(500).json({ message: 'Error extracting skills', skills: [] });
    }
});
